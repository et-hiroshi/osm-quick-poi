...
画面下部の5つのブランドボタンから選ぶと確認シートが開きます。「登録する」を押した時点の地図中央座標を固定し、changeset作成、node作成、changeset終了の順にOSM Editing APIへ送信します。送信中は二重送信を防止します。店舗名は既知ブランドでは任意、「その他」では必須です。セブン-イレブン、ファミリーマート、ローソン、ミニストップには日本向けName Suggestion Index準拠のタグを付けます。
...
地図操作の終了後、中央固定ピンから半径50m以内の `shop=convenience` をOverpass APIで検索します。nodeに加えて、way・relationも対象です。検索成功時に実際の結果がある場合だけ、登録確認シートに店名と距離を警告表示します。検索中・未検索・エラー・0件は登録を待たせません。
...
実機確認では、位置情報の許可・拒否・再試行、地図操作後の検索警告、任意／必須の店舗名、登録中の二重送信防止、ログイン表示と地図操作の非重複、Safe Area、ホーム画面起動を確認してください。
