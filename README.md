...
検索はHTTPS・CORS対応の `overpass-api.de` を使用し、混雑を示すHTTP 429/502/503/504またはタイムアウト時だけ、設定済みの `lz4.overpass-api.de` へ1回フォールバックします。検索要求不正、接続失敗、不正な応答では自動フォールバックせず、原因別のメッセージと再試行を表示します。
